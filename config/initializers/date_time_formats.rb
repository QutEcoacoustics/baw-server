...
Time::DATE_FORMATS[:readable_full_without_seconds] = lambda { |time| time.strftime("%a, #{ActiveSupport::Inflector.ordinalize(time.day)} %b %Y at %H:%M (#{time.formatted_offset(false)})") }
