      ...
      class CsvHelper
        class << self
          def logged_csv_line(file_path, exists, moved_path = nil,
            ...
            csv_values[30] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:file_hash) ? :updated : :noaction
              end
            csv_values[31] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:media_type) ? :updated : :noaction
              end
            csv_values[32] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:sample_rate_hertz) ? :updated : :noaction
              end
            csv_values[33] = if api_result_hash_blank
                               :notsent
                             else
                               api_result_hash.include?(:channels) ? :updated : :noaction
                             end
            csv_values[34] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:bit_rate_bps) ? :updated : :noaction
              end
            csv_values[35] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:data_length_bytes) ? :updated : :noaction
              end
            csv_values[36] =
              if api_result_hash_blank
                :notsent
              else
                api_result_hash.include?(:duration_seconds) ? :updated : :noaction
              end
