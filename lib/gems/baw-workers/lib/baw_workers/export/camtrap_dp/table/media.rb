      ...
      module Table
        ...
        class Media < ::Dry::Struct
          ...
          def self.mapping(audio_recording, deployment)
            ...
            # ? Should we be doing this for all of the timestamp related fields in the export?
            # ? E.g. in deployment accumulator (which already doesn't match the method used here)
            if deployment.site.timezone.blank? || deployment.site.timezone[:utc_total_offset].zero?
              audio_recording.recorded_date&.utc&.iso8601
            else
              timezone = TimeZoneHelper.tzinfo_class(deployment.site.tzinfo_tz)
              audio_recording.recorded_date.in_time_zone(timezone).iso8601
            end => timestamp
