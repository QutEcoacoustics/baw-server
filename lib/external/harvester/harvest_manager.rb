...
module Harvester
  ...
  class Manager
    ...
    # @param [string] yaml_settings_file
    def initialize(yaml_settings_file, dir_to_process)

      raise Exceptions::HarvesterConfigFileNotFound, "Configuration file not found: #{yaml_settings_file}" unless !yaml_settings_file.nil? && File.exists?(yaml_settings_file)
      raise Exceptions::HarvesterConfigurationError, "Directory to process not found: #{dir_to_process}" unless !dir_to_process.nil? && Dir.exists?(dir_to_process)
      ...
      begin
        ...
        Manager::Settings.source(yaml_settings_file)
