...
module AudioFfmpeg
  include OS, Logging
  @ffmpeg_path = if OS.windows? then "./vendor/bin/ffmpeg/ffmpeg.exe" else "ffmpeg" end
  @ffprobe_path = if OS.windows? then "./vendor/bin/ffmpeg/ffprobe.exe" else "ffprobe" end

  # public methods
  public

  # mime type to ffmpeg string identifier conversions
  def self.get_mime_type(ffmpeg_info)
