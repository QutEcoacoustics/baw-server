...
module Access
  ...
  module EffectivePermission
    ...
    def add_effective_permissions_cte(query, user, project_ids: nil, project_table: nil)
      ...
      cte_query = effective_permissions(user, project_ids:)
      query
        .with(TABLE_NAME => cte_query)
        .joins(
          project_table
    ...
    #
    # Returns a query with two columns: project_id and effective_level as an integer.
    # (See Access::Permission::LEVEL_TO_INTEGER_MAP for mapping of permission levels to integers.)
    # @param user [User]
    # @param project_ids [Array<Integer>] filter by project if it is specified
    # @return [Arel::SelectManager] Arel query that returns project_id and effective_level
    def effective_permissions(user, project_ids: nil)
      ...
      if user&.admin?
        ...
        return p
            ...
            .project(
              p[:id].as('project_id'),
              Arel.sql(owner_level.to_s).as('effective_level')
            )
      end
      ...
      query
        ...
        .project(
          pm[:project_id],
          case_statement.maximum.as('effective_level')
        )
    end

    def build_level_predicate(level)
      level = Access::Validate.level(level)

      level_value = Permission::LEVEL_TO_INTEGER_MAP[level]
      no_access_value = Permission::LEVEL_TO_INTEGER_MAP[Permission::NONE]

      TABLE[:effective_level].coalesce(no_access_value).gteq(level_value)
    end
  end
end
