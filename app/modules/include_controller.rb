...
module IncludeController
  ...
  def render_error(status_symbol, detail_message, error, method_name, options = {})
    ...
    if should_notify_error
      # in some case we want to notify of an error that was not generated by an exception.
      # These typically happen from routing errors.
      # In prod, exception notifier groups messages based on a CRC of the message.
      # If we supply a nil error, it will raise, so we always make sure error is not nil.
      error ||= StandardError.new("Nil error but details given: Status=#{status_symbol} Message=#{detail_message}")
