...
module Report
  module Cte
    class NodeTemplate < Node
      ...
      class << self
        alias table_name default_name=
        # alias options default_options=
        alias dependencies default_dependencies=
        alias select default_select=
        alias suffix default_suffix=
      end

      def options(**opts, &blk)
        @default_options = blk ? blk.call : opts
      end

      def dependencies(**deps, &blk)
        @default_dependencies = blk ? blk.call : deps
      end
