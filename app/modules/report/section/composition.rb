...
# This module defines a pipeline of Arel projections for summarising
# time-bucketed event composition data.

# The current implementation of Report::Section and steps is centered around
# CTEs and using Arel::Tables as constants. But here I had used TableAlias
# subqueries rather than CTEs, hence they are nested under one step. The
# step processing should be more flexible.
class Report::Section::Composition < Report::Section
  # TABLE_COMPOSITION_SERIES = Arel::Table.new('composition_series')
  TABLE_COMPOSITION_SERIES = :composition_series

  TABLES = {
    TABLE_COMPOSITION_SERIES => TABLE_COMPOSITION_SERIES
  }.freeze

  ROOT_NODE = TABLE_COMPOSITION_SERIES
  ...
  #
  # @return [Arel::SelectManager] the report query for the composition series
  step TABLE_COMPOSITION_SERIES do |options|
    # Outside dependencies - Passed from options
    bucketed_time_series = options[:bucketed_time_series]
    base_table = options[:base_table]
    base_verification = options[:base_verification]
    ...
    window = Arel::Nodes::Window.new.partition(bucketed_time_series[:bucket_number])
    ...
    Arel::SelectManager.new
      ...
      .on(bucketed_time_series[:time_bucket].contains(base_table[:start_time_absolute])
      ...
      .order(Arel.sql('distinct_tags.tag_id'), bucketed_time_series[:bucket_number])
  end

  # aggregates the composition series as JSON for reporting
  def project
    composition_series_aliased = collection[TABLE_COMPOSITION_SERIES].table.as('c')
    Arel::SelectManager.new
