...
class Report::Section::Coverage < Report::Section
  # the :analysis_result field in options is used to
  ...
  REQUIRED_TIME_SERIES_KEYS = [:start_time, :end_time, :bucket_size].freeze

  def validate_options!
    missing_keys = REQUIRED_TIME_SERIES_KEYS - options.keys
    raise ArgumentError, "Missing required TimeSeries options: #{missing}" unless missing_keys.empty?

    return unless options[:source].nil? || options[:lower_field].nil? || options[:upper_field].nil?

    raise ArgumentError, 'Missing required source, lower_field or upper_field in options'
  end

  TABLE_GAP_SIZE = :gap_size
  TABLE_SORT_WITH_LAG = :sort_with_lag
  TABLE_GROUPED = :grouped
  TABLE_COVERAGE_INTERVALS = :coverage_intervals
  TABLE_COVERAGE_EVENTS = :coverage_events
  TABLE_COVERAGE_EVENTS_SORTED_SUM = :coverage_events_sorted_sum
  TABLE_COVERED_INTERVALS = :covered_intervals
  TABLE_COVERED_DURATIONS = :covered_durations
  TABLE_FINAL_COVERAGE = :final_coverage
  ...
  step TABLE_GAP_SIZE do |options|
    # report boundaries
    calculate_gap_size(options)
  end

  step TABLE_SORT_WITH_LAG do |options|
    sort_with_lag(options)
  end

  step TABLE_GROUPED, depends_on: [TABLE_SORT_WITH_LAG, TABLE_GAP_SIZE] do |*depends_on, options|
    sorted_with_lag, gap_size_table = depends_on
    grouped(sorted_with_lag, gap_size_table, options)
  end

  step TABLE_COVERAGE_INTERVALS, depends_on: TABLE_GROUPED do |grouped, options|
    coverage_intervals(grouped, options)
  end

  step TABLE_COVERAGE_EVENTS, depends_on: TABLE_GROUPED do |grouped, options|
    coverage_events(grouped, options)
  end

  step TABLE_COVERAGE_EVENTS_SORTED_SUM, depends_on: TABLE_COVERAGE_EVENTS do |events, options|
    coverage_events_sorted_sum(events, options)
  end

  step TABLE_COVERED_INTERVALS, depends_on: TABLE_COVERAGE_EVENTS_SORTED_SUM do |events_sum, options|
    covered_intervals(events_sum, options)
  end

  step TABLE_COVERED_DURATIONS, depends_on: TABLE_COVERED_INTERVALS do |intervals, options|
    covered_durations(intervals, options)
  end

  step TABLE_FINAL_COVERAGE, depends_on: [TABLE_COVERAGE_INTERVALS, TABLE_COVERED_DURATIONS] do |*depends_on, options|
  ...
  class << self
    def calculate_gap_size(options = {})
      start_time, end_time, scaling = options.values_at(:start_time, :end_time, :scaling_factor)
