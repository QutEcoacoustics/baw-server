...
module Report
  module Section
    module Accumulation
      ...
      TABLE_TIME_RANGE_AND_INTERVAL = Arel::Table.new('time_range_and_interval')
      TABLE_NUMBER_OF_BUCKETS = Arel::Table.new('number_of_buckets')
      TABLE_BUCKETED_TIME_SERIES = Arel::Table.new('bucketed_time_series')
      TABLE_DATA_WITH_ALLOCATED_BUCKET = Arel::Table.new('data_with_allocated_bucket')
      TABLE_TAG_FIRST_APPEARANCE = Arel::Table.new('tag_first_appearance')
      TABLE_SUM_UNIQUE_TAGS_BY_BUCKET = Arel::Table.new('sum_unique_tags_by_bucket')
      TABLE_CUMULATIVE_UNIQUE_TAG_SERIES = Arel::Table.new('cumulative_unique_tag_series')
      ...
      TABLES = {
        ...
        cumulative_unique_tag_series: TABLE_CUMULATIVE_UNIQUE_TAG_SERIES
      }.freeze
      ...
      step table: TABLE_TIME_RANGE_AND_INTERVAL, as: proc { |_t, options|
        time_range_and_interval_query(options)
      }

      step table: TABLE_NUMBER_OF_BUCKETS, depends_on: TABLE_TIME_RANGE_AND_INTERVAL, as: proc { |_t, time_range_and_interval, options|
        number_of_buckets_query(time_range_and_interval, options[:interval])
      }

      step table: TABLE_BUCKETED_TIME_SERIES, depends_on: TABLE_NUMBER_OF_BUCKETS, as: proc { |_t, number_of_buckets|
        bucketed_time_series_query(number_of_buckets)
      }

      step table: TABLE_DATA_WITH_ALLOCATED_BUCKET, depends_on:  TABLE_NUMBER_OF_BUCKETS, as: proc { |_t, number_of_buckets|
        allocate_bucket_based_on_start_time_abosolute(AudioEventReport::BASE_TABLE, number_of_buckets)
      }

      step table: TABLE_TAG_FIRST_APPEARANCE, depends_on: TABLE_DATA_WITH_ALLOCATED_BUCKET, as: proc { |_t, data_with_allocated_bucket|
        tag_first_appearance_query(data_with_allocated_bucket)
      }

      step table: TABLE_SUM_UNIQUE_TAGS_BY_BUCKET, depends_on: TABLE_TAG_FIRST_APPEARANCE, as: proc { |_t, tag_first_appearance|
        sum_unique_tags_by_bucket_query(tag_first_appearance)
      }

      step table: TABLE_CUMULATIVE_UNIQUE_TAG_SERIES, depends_on: [TABLE_BUCKETED_TIME_SERIES, TABLE_SUM_UNIQUE_TAGS_BY_BUCKET], as: proc { |_t, bucketed_time_series, sum_unique_tags_by_bucket|
        cumulative_unique_tag_series_query(bucketed_time_series, sum_unique_tags_by_bucket)
      }
      ...
      def project(_collection = nil, _options = nil)
        cumulative_table = TABLE_CUMULATIVE_UNIQUE_TAG_SERIES
        cumulative_table_aliased = cumulative_table.as('t')
        Arel::SelectManager.new
          .project(cumulative_table_aliased.right.row_to_json.json_agg)
          .from(cumulative_table_aliased)
      end

      # TODO: rename start_time => lower, end_time => upper, starting with TS.options
      #
