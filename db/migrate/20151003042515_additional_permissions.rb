class AdditionalPermissions < ActiveRecord::Migration
  def up
    ...
    Project.all.each do |project|
      ...
      fail RuntimeError, "More than one permission for one user and project! #{existing_permissions.all.to_json}" if existing_permissions_count > 1
...
CHECK ((user_id IS NOT NULL AND NOT allow_logged_in AND NOT allow_anonymous)
    ...
    add_index  :permissions, [:project_id, :allow_logged_in], unique: true, where: 'allow_logged_in', name: 'permissions_project_allow_logged_in_uidx'

    # ensure each project has only one record that is allow_anonymous: true
    add_index :permissions, [:project_id, :allow_anonymous], unique: true, where: 'allow_anonymous', name: 'permissions_project_allow_anonymous_uidx'
  end
