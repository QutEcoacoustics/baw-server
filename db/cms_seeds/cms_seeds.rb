...
cms_site = Comfy::Cms::Site.where(identifier: DEFAULT_SITE).first
cms_site = Comfy::Cms::Site.create!(identifier: DEFAULT_SITE, hostname: 'localhost') if cms_site.nil?
...
index = create_page(cms_site, PAGES[:index])

PAGES.except(:index).each do |_key, page_hash|
