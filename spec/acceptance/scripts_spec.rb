...
resource 'Scripts' do
  ...
  get '/scripts/?filter_is_last_version=:filter_is_last_version' do
    ...
    standard_request_options(
      ...
      &proc { |context, opts|
        opts[:response_body_content][3] =
          format(opts[:response_body_content][3], version: context.script[:version])
        opts[:response_body_content][4] =
          format(opts[:response_body_content][4], group_id: context.extra_scripts.last[:group_id])
      }
    )
  end
