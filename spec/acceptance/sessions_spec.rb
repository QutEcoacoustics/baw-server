...
resource 'Sessions' do
  ...
  post '/security' do
    parameter :email, 'owner_user email (must provide one of email or login)', required: false
    parameter :login, 'owner_user name or email (must provide one of email or login)', required: false
    parameter :password, 'owner_user password', required: true
  ...
  post '/security' do
    ...
    standard_request_options(:post, 'Sign In (reader using login: owner_user name)', :ok, {response_body_content: I18n.t('devise.sessions.signed_in'), expected_json_path: 'data/auth_token'})
  end
  ...
  get '/security/owner_user?user_token=:authentication_token' do
  ...
  get '/security/owner_user?email=:email&password=:password' do
  ...
  get '/security/owner_user?login=:login&password=:password' do
  ...
  get '/security/owner_user?login=:login&password=:password' do
    ...
    let(:password) { Rack::Utils.escape(reader_user_password) }
    standard_request_options(:get, 'Get Token with login (owner_user name)/pass auth in qsp (reader)', :ok, {response_body_content: 'confirmed_user', expected_json_path: 'data/auth_token'})
  end
