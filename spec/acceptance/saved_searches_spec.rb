...
resource 'SavedSearches' do
  ...
  context 'update' do
    put '/saved_searches/:id' do
      ...
      let(:authentication_token) { writer_token }
      standard_request_options(:put, 'UPDATE (as writer)', :not_found,
        { expected_json_path: 'meta/error/info/original_http_method' })
    end
    ...
    patch '/saved_searches/:id' do
      ...
      let(:raw_post) { { audio_event_comment: post_attributes }.to_json }
      let(:authentication_token) { reader_token }
      standard_request_options(:patch, 'UPDATE (as reader)', :not_found,
        { expected_json_path: 'meta/error/info/original_http_method' })
    end
  end
