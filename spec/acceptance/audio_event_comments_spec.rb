...
resource 'AudioEventComments' do
  ...
  before(:each) do
    ...
    @write_permission = FactoryGirl.create(:write_permission, user: @user_1, creator: @user_1)

    @user_2 = FactoryGirl.create(:user)
    @read_permission = FactoryGirl.create(:read_permission, project: @write_permission.project, user: @user_2, creator: @user_2)

    # the two comments have the same creator/updater
    # need to have different users
    # also, controller actions need to load audio_event_comment
    audio_event = @write_permission.project.sites[0].audio_recordings[0].audio_events[0]
    @comment_1 = audio_event.audio_event_comments[0]
  ...
  # LIST
  ################################
  get '/audio_events/:audio_event_id/audio_event_comments' do
  ...
  # CREATE
  ################################
  post '/audio_events/:audio_event_id/audio_event_comments' do
    parameter :audio_event_id, 'Requested audio event id (in path/route)', required: true
    let(:audio_event_id) { @comment_1.audio_event_id }
  ...
  # Update
  ################################
  put '/audio_events/:audio_event_id/audio_event_comments/:id' do
    parameter :audio_event_id, 'Requested audio event id (in path/route)', required: true
  ...
  put '/audio_events/:audio_event_id/audio_event_comments/:id' do
    ...
    let(:raw_post) { {audio_event_comment: post_attributes}.to_json }
    let(:authentication_token) { reader_token }
    standard_request('UPDATE (as reader)', 204, nil, true)
  end
  ...
  # Destroy
  ################################
  delete '/audio_events/:audio_event_id/audio_event_comments/:id' do
    parameter :audio_event_id, 'Requested audio event id (in path/route)', required: true
    parameter :id, 'Requested audio event comment id (in path/route)', required: true
    let(:audio_event_id) { @comment_1.audio_event_id }
    let(:id) { @comment_1.id }
    let(:raw_post) { {audio_event_comment: post_attributes}.to_json }
    let(:authentication_token) { writer_token }
    standard_request('DESTROY (as writer)', 204, nil, true)
  end
  ...
  delete '/audio_events/:audio_event_id/audio_event_comments/:id' do
    ...
    let(:raw_post) { {audio_event_comment: post_attributes}.to_json }
    let(:authentication_token) { reader_token }
    standard_request('DESTROY (as reader)', 204, nil, true)
  end
