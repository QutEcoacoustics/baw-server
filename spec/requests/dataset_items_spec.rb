...
describe 'Dataset Items' do
  ...
  let(:dataset_item_attributes) {
    attributes_for(:dataset_item, { audio_recording_id: audio_recording.id })
  }
  ...
  describe 'delete' do
    it 'deletes dataset item and any children' do
      dataset_item_count = DatasetItem.count
      progress_event_count = ProgressEvent.count
      ...
      expect(DatasetItem.count).to eq(dataset_item_count - 1)
      # ensure children are deleted
      expect(ProgressEvent.count).to eq(expected_progress_event_count)
      expect(Response.count).to eq(expected_response_count)
    end
  end

  # AT 2024: intentionally broken - we're no longer using datasets
  describe 'filter', pending: 'intentionally broken deprecated' do
  ...
  # AT 2024: intentionally broken - we're no longer using datasets
  describe 'list next for me', pending: 'intentionally broken deprecated' do
    ...
    def filter_and_sort_for_comparison(items, user, page = nil, limit = nil)
      ...
      sorted_ids[from_index..to_index]
    end
      ...
      combos.each do |combo|
        ...
        it "orders items correctly for #{users_description[u]} page #{page} limit #{limit}" do
          ...
          if paging_response
            ...
            { page: expected_page, items: expected_items, total: expected_total,
              max_page: expected_max_page }
            paging_numbers.should
            ...
            if expected_page < expected_max_page
              ...
              expect(paging_response['next']).to be_nil
            end
            ...
            if expected_page > 1 && expected_page <= expected_max_page
              ...
              expect(paging_response['previous']).to be_nil
            end
