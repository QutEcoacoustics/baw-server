...
describe Permission, type: :model do
  subject { build(:read_permission, project: create(:project)) }

  it do
    is_expected.to belong_to(:creator).class_name('User').inverse_of(:created_permissions)
  end

  it do
    is_expected.to \
      belong_to(:updater).class_name('User').inverse_of(:updated_permissions).optional
  end
  ...
  context 'special tests' do
    ...
    context 'enforces exactly one permission setting per project' do
      context 'fails with' do
        it 'no settings' do
          permission = build(:permission, level: 'reader', creator: user,
        ...
        it 'user at invalid level' do
          permission = build(:permission, level: 'something_wrong', creator: user,
            user: user, allow_logged_in: false, allow_anonymous: false)
          expect(permission).not_to be_valid
        end

        it 'user at nil level' do
          permission = build(:permission, level: nil, creator: user,
            user: user, allow_logged_in: false, allow_anonymous: false)
          expect(permission).not_to be_valid
        end
      end
      ...
      context 'succeeds with' do
        ...
        it 'user at writer level' do
          permission = build(:permission, level: 'writer', creator: user,
            user: user, allow_logged_in: false, allow_anonymous: false)
          expect(permission).to be_valid
        end

        it 'user at owner level' do
          permission = build(:permission, level: 'owner', creator: user,
            user: user, allow_logged_in: false, allow_anonymous: false)
          expect(permission).to be_valid
        end
      end
