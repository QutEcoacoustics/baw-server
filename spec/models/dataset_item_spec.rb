...
RSpec.describe DatasetItem, type: :model do
  ...
  subject { FactoryGirl.build(:dataset_item) }
  it 'has a valid factory' do
    expect(FactoryGirl.create(:dataset_item)).to be_valid
  end
  ...
  it 'is invalid if order is empty' do
  ...
  it 'is valid if order is numeric' do
    ...
    expect(build(:dataset_item, {order: 2000000000.1})).to be_valid
  end
