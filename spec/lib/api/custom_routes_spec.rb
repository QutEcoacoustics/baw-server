...
describe Api::CustomRoutes do
  let!(:application_routes) { ActionDispatch::Routing::RouteSet.new }
  let!(:route) { route_builder('GET', 'projects/something', 'projects#something', application_routes) }
  let!(:devise_route) { route_builder('GET', 'my_account/cancel', 'devise/registrations#cancel', application_routes)}
  let!(:internal_route) { route_builder('GET', '/assets', 'rails#get_info', application_routes) }

  describe '#filtered_routes' do
    let(:routes) { Api::CustomRoutes.new(routes: application_routes.routes) }
  ...
  describe '#api_specs' do
    subject(:route_check) { Api::CustomRoutes.new(world: world) }
  ...
  describe '#missing_docs' do
    subject(:route_check) { Api::CustomRoutes.new(routes: application_routes.routes, world: world) }

    let(:configuration) { RSpec::Core::Configuration.new }
    let(:world) { RSpec::Core::World.new(configuration) }
    let(:wrapped_route) { ActionDispatch::Routing::RouteWrapper.new(route) }
    let(:formatted_route) { ::Route.new(wrapped_route.verb, wrapped_route.path[/\/[^( ]+/]) }
    ...
    it 'does not return routes for which an api spec exists' do
      group = RSpec::Core::ExampleGroup.describe('resource group', {api_doc_dsl: :resource}) do
        context('/path/to/api', {api_doc_dsl: :endpoint, method: :get, route: '/projects/something'})
      end
  ...
  def route_builder(method, path, action, route_set)
    ...
    options = {via: method, to: action, anchor: true, as: name, format: false}

    mapping = ActionDispatch::Routing::Mapper::Mapping.new(scope, set, path, defaults, as, options)
