...
describe BawWorkers::ActiveJob::Identity::Helpers do
  before do
  ...
  it 'can generate a uuid' do
    result = BawWorkers::ActiveJob::Identity::Helpers.generate_uuid(job)
    expect(result).to match /FakeJob:[-a-f0-9]{36}/
  end

  it 'can generate a deterministic hash' do
    result = BawWorkers::ActiveJob::Identity::Helpers.generate_hash_id(job)
  ...
  it 'can generate a deterministic hash with a different order of args' do
    ...
    result = BawWorkers::ActiveJob::Identity::Helpers.generate_hash_id(job_with_different_order_args)
    expect(result).to eq('FakeJob:c1dc6d8b3863caabc2bd32d342f01b83')
  end

  it 'can template a hash string into a key' do
    result = BawWorkers::ActiveJob::Identity::Helpers.generate_keyed_id(job, { a: 1, b: 'hello' })
  ...
  it 'will replace non alpha-numeric characters complex values in generate_keyed_id' do
    result = BawWorkers::ActiveJob::Identity::Helpers.generate_keyed_id(job, { a: 1, b: { some: 'complex_hash' } })
  ...
  it 'will fail if it generates a key that is too long in generate_keyed_id' do
    expect {
      BawWorkers::ActiveJob::Identity::Helpers.generate_keyed_id(job, { a: 'banana' * 1000 })
